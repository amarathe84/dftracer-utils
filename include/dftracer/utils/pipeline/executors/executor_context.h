...
public:
    ...
    TaskIndex add_dynamic_task(std::unique_ptr<Task> task, TaskIndex depends_on = -1);
    void add_dynamic_dependency(TaskIndex from, TaskIndex to);
    
    Task* get_dynamic_task(TaskIndex index) const;
    const std::vector<TaskIndex>& get_dynamic_dependencies(TaskIndex index) const;
...
private:
    ...
    std::vector<std::vector<TaskIndex>> dynamic_dependencies_;  // who depends on this task  
    std::vector<std::vector<TaskIndex>> dynamic_dependents_;    // who this task depends on
