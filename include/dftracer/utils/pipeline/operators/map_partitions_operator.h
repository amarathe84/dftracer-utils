...
class MapPartitionsOperator : public Operator {
 public:
  struct PartitionInfo {
    std::size_t partition_index = 0;        // index of this partition within the current context
    std::size_t partitions_in_context = 1;  // total partitions executed in this context
    std::size_t upstream_offset_elems = 0;  // starting element offset into the logical upstream stream
    std::size_t upstream_count_elems = 0;   // number of input elements in this partition
    std::size_t world_rank = 0;             // process/rank id (0 if single-process)
    std::size_t world_size = 1;             // number of processes/ranks (1 if single-process)
  };

  using Fn = void (*)(const PartitionInfo& part,
  ...
  MapPartitionsOperator(std::size_t in_elem_sz,
                        std::size_t out_elem_sz,
