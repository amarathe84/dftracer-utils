...
class MPIContext : public ExecutionContext<MPIContext> {
 private:
  ...
  std::vector<char> serialize_strings(
  ...
  std::vector<std::string> deserialize_strings(
  ...
  std::vector<std::string> gather_strings(
      const std::vector<std::string>& local_strings) const {
    ...
    MPI_Allgatherv(local_data.data(), local_size, MPI_CHAR, all_data.data(),
                   sizes.data(), displacements.data(), MPI_CHAR, comm_);
  ...
  std::vector<char> serialize_key_value_pair(
      const KeyT& key, const std::vector<InT>& values) const {
    ...
    if constexpr (std::is_arithmetic_v<InT>) {
      ...
      buffer.insert(buffer.end(), values_bytes,
                    values_bytes + values.size() * sizeof(InT));
  ...
  template <typename KeyT, typename InT>
  std::pair<KeyT, std::vector<InT>> deserialize_key_value_pair(
  ...
  template <typename KeyT, typename InT>
  std::unordered_map<KeyT, std::vector<InT>> redistribute_by_key(
      const std::unordered_map<KeyT, std::vector<InT>>& local_groups) const {
    ...
    size_t offset = 0;
    for (int i = 0; i < size_; ++i) {
      std::memcpy(send_buffer.data() + offset, send_buffers[i].data(),
                  send_counts[i]);
    ...
    MPI_Alltoallv(send_buffer.data(), send_counts.data(), send_displs.data(),
                  MPI_CHAR, recv_buffer.data(), recv_counts.data(),
                  recv_displs.data(), MPI_CHAR, comm_);
    ...
    while (offset < recv_buffer.size()) {
      ...
      existing_values.insert(existing_values.end(), values.begin(),
                             values.end());
 ...
 public:
  ...
  std::vector<OutT> map_impl(MapFn&& fn, const std::vector<InT>& input) const {
    ...
    if constexpr (std::is_arithmetic_v<OutT>) {
      ...
      MPI_Allgatherv(local_result.data(), local_count, get_mpi_datatype<OutT>(),
                     global_result.data(), recv_counts.data(),
                     displacements.data(), get_mpi_datatype<OutT>(), comm_);
  ...
  OutT reduce_impl(ReduceFn&& fn, const std::vector<InT>& input) const {
    ...
    auto [start, end] = get_chunk_bounds(total_size);
    std::vector<InT> local_chunk(local_input.begin() + start,
                                 local_input.begin() + end);
