...
public:
    ...
    std::vector<OutT> map_impl(MapFn&& fn, const std::vector<InT>& input) const {
        ...
        for (size_t i = 0; i < input.size(); i += chunk_size) {
            ...
            futures.emplace_back(std::async(std::launch::async, [&fn, &input, i, end]() {
    ...
    OutT reduce_impl(ReduceFn&& fn, const std::vector<InT>& input) const {
        // For reduce, we just call the function directly since it operates on the whole vector
        return fn(input);
    }
    ...
    OutT aggregate_impl(ChunkFn&& chunk_fn, AggFn&& agg_fn, FinalizeFn&& finalize_fn,
        ...
        for (size_t i = 0; i < input.size(); i += chunk_size) {
            ...
            chunk_futures.emplace_back(std::async(std::launch::async, 
                [&chunk_fn, &input, i, end]() {
    ...
    std::unordered_map<KeyT, OutT> groupby_aggregate_impl(KeyFn&& key_fn, AggFn&& agg_fn,
        ...
        const size_t chunk_size = std::max(size_t(1), input.size() / num_threads_);
        std::vector<std::future<std::unordered_map<KeyT, std::vector<InT>>>> futures;
