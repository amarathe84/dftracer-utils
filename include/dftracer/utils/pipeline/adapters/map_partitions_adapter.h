...
constexpr bool is_emitter_form_mp_v =
    std::is_invocable_v<Fn, const Part&, const In*, std::size_t,
                        EmitProbe<Out>>;

template <class Fn, class In, class Out>
constexpr bool is_vector_form_mp_v =
    std::is_invocable_r_v<std::vector<Out>, Fn, const Part&, const In*,
                          std::size_t>;

template <class Fn, class In, class Out>
constexpr bool is_init_list_form_mp_v =
    std::is_invocable_r_v<std::initializer_list<Out>, Fn, const Part&,
                          const In*, std::size_t>;

template <class Fn, class In, class Out>
constexpr bool is_ptr_count_form_mp_v =
    std::is_invocable_r_v<std::pair<const Out*, std::size_t>, Fn, const Part&,
                          const In*, std::size_t>;
}  // namespace detail
...
struct MPStateEmitter {
  ...
  static void tramp(const Part& part, const void* in_partition,
                    std::size_t in_count, std::size_t in_elem_size,
                    void* out_partition, std::size_t* out_count,
                    std::size_t out_elem_size, void* st_void) {
...
struct MPStateVector {
  ...
  static void tramp(const Part& part, const void* in_partition,
    ...
    *out_count = need;
    ...
    if (ncpy) std::memcpy(out, ys.data(), ncpy * sizeof(Out));
  }
};
...
struct MPStatePtrCount {
  ...
  static void tramp(const Part& part, const void* in_partition,
    ...
    const std::size_t need = view.second;
    const std::size_t cap = *out_count;
    *out_count = need;
    const std::size_t ncpy = need < cap ? need : cap;
...
inline OpHandler<operators::MapPartitionsOperator> make_map_partitions_op(
    Fn fn) {
  OpHandler<operators::MapPartitionsOperator> h{
      operators::MapPartitionsOperator(sizeof(In), sizeof(Out)), nullptr};
