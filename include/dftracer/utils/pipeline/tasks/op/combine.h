    ...
    std::any execute(std::any input) override {
        ...
        if (inputs.size() != sizeof...(InputTypes)) {
            throw PipelineError(PipelineError::VALIDATION_ERROR,
                ...
                std::to_string(sizeof...(InputTypes)) + ", Got: " + 
                std::to_string(inputs.size()));
        }
        
        return std::any(unpack_and_call(inputs, std::index_sequence_for<InputTypes...>{}));
    }
    
private:
    template<size_t... Is>
    Result unpack_and_call(const std::vector<std::any>& inputs, std::index_sequence<Is...>) {
        try {
            return combiner_(std::any_cast<InputTypes>(inputs[Is])...);
        } catch (const std::bad_any_cast& e) {
            throw PipelineError(PipelineError::TYPE_MISMATCH_ERROR,
                "Type mismatch in CombineTask input unpacking: " + std::string(e.what()));
        }
    }
};
