...
struct PipelineOutput : public std::unordered_map<TaskIndex, std::any> {
    operator std::any() const {
        ...
        throw PipelineError(PipelineError::OUTPUT_CONVERSION_ERROR, 
                           "Cannot convert multi-output pipeline (" + std::to_string(size()) + " outputs) to single std::any");
    }

    std::any get() const {
        if (size() == 1) return begin()->second;
        throw PipelineError(PipelineError::OUTPUT_CONVERSION_ERROR,
                           "Cannot get single value from multi-output pipeline (" + std::to_string(size()) + " outputs)");
    }
    ...
    T get(TaskIndex id) const {
      ...
      try {
          ...
          throw PipelineError(PipelineError::OUTPUT_CONVERSION_ERROR,
                             "Failed to cast output of task ID " + std::to_string(id) + ": " + e.what());
      }
    }
    
    std::any first() const {
        if (empty()) throw PipelineError(PipelineError::OUTPUT_CONVERSION_ERROR, 
