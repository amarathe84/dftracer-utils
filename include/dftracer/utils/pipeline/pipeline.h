...
template <typename PrevOp, typename ChunkFn, typename AggFn,
          typename FinalizeFn, typename InT, typename ChunkT, typename AggT,
          typename OutT>
struct AggregateOp {
  PrevOp prev_op;
  ChunkFn chunk_fn;
...
template <typename PrevOp, typename KeyFn, typename AggFn, typename InT,
          typename KeyT, typename OutT>
struct GroupByOp {
  PrevOp prev_op;
  KeyFn key_fn;
  AggFn agg_fn;
 ...
 public:
  ...
  auto map(MapFn&& fn) const {
    return Pipeline<InputT, OutT, MapOp<Operation, MapFn, OutputT, OutT>>{
        MapOp<Operation, MapFn, OutputT, OutT>{operation_,
                                               std::forward<MapFn>(fn)}};
  }
  ...
  auto reduce(ReduceFn&& fn) const {
    return Pipeline<InputT, OutT, ReduceOp<Operation, ReduceFn, OutputT, OutT>>{
        ReduceOp<Operation, ReduceFn, OutputT, OutT>{
            operation_, std::forward<ReduceFn>(fn)}};
  }

  template <typename ChunkT, typename AggT, typename OutT, typename ChunkFn,
            typename AggFn, typename FinalizeFn>
  auto aggregate(ChunkFn&& chunk_fn, AggFn&& agg_fn,
                 FinalizeFn&& finalize_fn) const {
    return Pipeline<InputT, OutT,
                    AggregateOp<Operation, ChunkFn, AggFn, FinalizeFn, OutputT,
                                ChunkT, AggT, OutT>>{
        AggregateOp<Operation, ChunkFn, AggFn, FinalizeFn, OutputT, ChunkT,
                    AggT, OutT>{operation_, std::forward<ChunkFn>(chunk_fn),
  ...
  auto groupby(KeyFn&& key_fn, AggFn&& agg_fn) const {
    return Pipeline<InputT, std::unordered_map<KeyT, OutT>,
        ...
        GroupByOp<Operation, KeyFn, AggFn, OutputT, KeyT, OutT>{
            operation_, std::forward<KeyFn>(key_fn),
            std::forward<AggFn>(agg_fn)}};
  }
...
struct ComposedOp {
  ...
  auto operator()(const Context& context, Input&& input) const {
    return apply_ops(context, std::forward<Input>(input),
                     std::index_sequence_for<Ops...>{});
  }

 private:
  template <typename Context, typename Input, std::size_t... Is>
  auto apply_ops(const Context& context, Input&& input,
...
auto compose(Ops&&... ops) {
  return ComposedOp<std::decay_t<Ops>...>{
