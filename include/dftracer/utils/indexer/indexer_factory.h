...
class IndexerFactory {
   public:
     ...
     * Automatically detects the format (GZIP vs TAR.GZ) and creates the appropriate indexer.
     * 
     * @param archive_path Path to the archive file (.gz or .tar.gz)
     * @param idx_path Path to the index file (optional - will be auto-generated if empty)
     * @param checkpoint_size Checkpoint size in bytes
     * @param force Force rebuilding the index even if it exists
     * @return Unique pointer to the appropriate indexer, or nullptr if format not supported
     */
    static std::unique_ptr<Indexer> create(
        const std::string &archive_path,
        const std::string &idx_path = "",
        std::uint64_t checkpoint_size = constants::indexer::DEFAULT_CHECKPOINT_SIZE,
        bool force = false);
     ...
     */
    static gzip_common::ArchiveFormat detect_format(const std::string &archive_path);
     ...
     */
    static std::string generate_index_path(const std::string &archive_path, 
