...
struct TraceRecord {
    ...
    void serialize(Archive& ar) {
        ar(cat, io_cat, acc_pat, func_name, duration, count, size, time_range,
           time_start, time_end, epoch, pid, tid, fhash, hhash, image_id,
           offset, event_type, view_fields, bin_fields);
    }
};
...
inline auto load_traces(Context& ctx, const std::vector<std::string>& traces,
    ...
    return generate_chunks(ctx, traces, batch_size)
        ...
        .map_partitions([](const auto& partition) -> OwnedJsonDocuments {
            ...
            spdlog::debug(
                "Processing partition with {} work items on thread: {}",
                partition.size(),
            ...
            for (const auto& work : partition) {
                try {
                    dftracer::utils::reader::Reader reader(work.path,
                                                           work.path + ".idx");
                    auto lines = reader.read_json_lines_bytes_owned(work.start,
                                                                    work.end);

                    spdlog::trace("Read {} JSON documents from {}:{}-{}",
                                  lines.size(), work.path, work.start,
                                  work.end);
            ...
            spdlog::debug("Partition complete: {} total documents",
                          results.size());
            return results;
        });
}
...
inline auto parse_and_filter_traces(
    ...
    return trace_reader::load_traces(ctx, traces, batch_size)
        .repartition("64MB")  // Repartition after JSON parsing for better
                              // memory distribution
        .map_partitions([view_types, config](
            ...
            spdlog::trace("Parsed {} valid trace records",
                          valid_records.size());
    ...
    auto hash_pairs = trace_records.flatmap(
        [](const TraceRecord& record)
            ...
            if (record.event_type == 1 && !record.fhash.empty()) {  // file hash
                hash_mappings.emplace_back("file:" + record.fhash,
                                           record.func_name);
            } else if (record.event_type == 2 &&
                       !record.hhash.empty()) {  // host hash
                hash_mappings.emplace_back("host:" + record.hhash,
                                           record.func_name);
            }
...
inline auto separate_events_and_hashes(
    ...
    return std::forward<BagType>(trace_records)
        .map_partitions([file_hash_map, host_hash_map](
                            const std::vector<TraceRecord>& partition)
            ...
            for (auto record : partition) {
                if (record.event_type == 0) {  // regular event only
                    ...
                    record.view_fields["proc_name"] =
                        "app#" + host_name + "#" + std::to_string(record.pid) +
                    ...
                    if (!file_name.empty() &&
                        ...
                        if (file_name.find("/checkpoint") !=
                            std::string::npos) {
                            record.cat = record.cat + "_checkpoint";
                        } else if (file_name.find("/data") !=
                                   std::string::npos) {
                            record.cat = record.cat + "_reader";
                        } else if (file_name.find("/lustre") !=
                                   std::string::npos) {
                            record.cat = record.cat + "_lustre";
                        } else if (file_name.find("/ssd") !=
                                   std::string::npos) {
                    ...
                    if (!file_name.empty()) {
                        for (const auto& pattern :
                             constants::IGNORED_FILE_PATTERNS) {
...
inline auto postread_trace(Context& ctx, BagType&& events,
    ...
    // PHASE 2: Apply epoch assignment using map_partitions for consistent
    // return type
    return std::forward<BagType>(events).map_partitions([epoch_spans,
                                                         view_types](
                                                            const std::vector<
                                                                TraceRecord>&
                                                                partition)
            ...
            [hlm_groupby_set, view_types_diff](
                ...
                for (const auto& record : records) {
                    ...
                    if (record.size.has_value()) {
                        ...
                        hlm.size_sum =
                            hlm.size_sum.value() + record.size.value();
                    }
                ...
                if (!records.empty()) {
                    ...
                    hlm.group_values["epoch"] =
                        std::to_string(first_record.epoch);

                    for (const auto& [field, value] :
                         first_record.view_fields) {
...
std::vector<HighLevelMetrics> restore_view(
    ...
    if (write_to_disk) {
        ...
        if (!status.ok()) {
            ...
            spdlog::info("Successfully wrote {} HLMs to checkpoint",
                         hlms.size());
        }
    }
    ...
    try {
        ...
        auto hlms = pipeline::restore_view<Context>(
            ...
            [&]() -> std::vector<HighLevelMetrics> {
                ...
                auto events = pipeline::read_traces(ctx, traces, config_,
                                                    config_.checkpoint_size(),
                                                    proc_view_types);

                // Step 2: Apply global timestamp normalization
                auto normalized_events =
                    pipeline::normalize_timestamps_globally(ctx, events,
                                                            config_);
                ...
                return pipeline::compute_high_level_metrics(
