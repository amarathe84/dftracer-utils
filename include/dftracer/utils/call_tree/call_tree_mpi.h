...
public:
     ...
     */
    static std::unique_ptr<internal::CallTree> load(const std::string& filename);
    
    /**
     * Get the generated call graph
     * @return Reference to the call graph
     */
    internal::CallTree& get_call_graph() { return *call_graph_; }
    const internal::CallTree& get_call_graph() const { return *call_graph_; }
...
private:
    ...
    std::unique_ptr<internal::CallTree> call_graph_;
    ...
    SerializableProcessGraph convert_to_serializable(const internal::ProcessCallTree& graph) const;
...
struct CallGraphBuildTask {
    ...
    internal::ProcessCallTree execute(internal::CallTree& graph);
};
