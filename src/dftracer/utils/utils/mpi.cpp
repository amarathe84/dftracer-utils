...
void MPI::send(const void* data, int count, MPI_Datatype datatype, int dest, int tag) {
    int error = MPI_Send(data, count, datatype, dest, tag, MPI_COMM_WORLD);
    check_error(error, "MPI_Send");
}

void MPI::recv(void* data, int count, MPI_Datatype datatype, int source, int tag, MPI_Status* status) {
    MPI_Status local_status;
    MPI_Status* status_ptr = status ? status : &local_status;
    
    int error = MPI_Recv(data, count, datatype, source, tag, MPI_COMM_WORLD, status_ptr);
...
std::vector<uint8_t> MPI::broadcast_vector(const std::vector<uint8_t>& data, int root) {
