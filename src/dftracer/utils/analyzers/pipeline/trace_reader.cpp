...
Pipeline TraceReader::build() {
    ...
    // Create individual metadata tasks for each trace file (source tasks, no dependencies)  
    for (size_t file_idx = 0; file_idx < traces.size(); ++file_idx) {
        ...
        auto metadata_task = Tasks::map<std::string, FileMetadata>(
            [trace_path, file_idx](const std::string& input_path) -> FileMetadata {
                auto thread_id = std::this_thread::get_id();
                // Only process when input_path matches this task's assigned file
                if (input_path == trace_path) {
                    DFTRACER_UTILS_LOG_ERROR("[Thread %zu] Processing metadata for file [%zu]: %s", std::hash<std::thread::id>{}(thread_id), file_idx, trace_path.c_str());
                    Indexer indexer(trace_path, trace_path + ".idx");
                    indexer.build();
                    auto max_bytes = indexer.get_max_bytes();
                    DFTRACER_UTILS_LOG_ERROR("[Thread %zu] Max bytes for %s: %zu", std::hash<std::thread::id>{}(thread_id), trace_path.c_str(), max_bytes);
                    return FileMetadata{trace_path, max_bytes};
                } else {
                    // Return empty metadata for non-matching files (will be filtered out downstream)
    ...
    // Create chunk generation tasks for each file (parallel, depends on respective metadata)
    for (size_t i = 0; i < traces.size(); ++i) {
        auto chunks_task = Tasks::flatmap<FileMetadata, WorkInfo>(
            [&](const FileMetadata& file_info) -> std::vector<WorkInfo> {
                ...
                DFTRACER_UTILS_LOG_DEBUG("[Thread %zu] Creating chunks for file: %s, size: %zu", std::hash<std::thread::id>{}(thread_id), file_info.path.c_str(), file_info.size);
                ...
                DFTRACER_UTILS_LOG_DEBUG("[Thread %zu] Created %zu work items for %s", std::hash<std::thread::id>{}(thread_id), work_items.size(), file_info.path.c_str());
                return work_items;
            }
        );
    ...
    for (size_t i = 0; i < chunks_indices.size(); ++i) {
        auto load_task = Tasks::flatmap<WorkInfo, json::OwnedJsonDocuments>(
            [](const WorkInfo& work) -> std::vector<json::OwnedJsonDocuments> {
                ...
                DFTRACER_UTILS_LOG_DEBUG("[Thread %zu] Loading work item: %s [%zu-%zu]", std::hash<std::thread::id>{}(thread_id), work.path.c_str(), work.start, work.end);
                std::vector<json::OwnedJsonDocuments> results;
                
                try {
                    Reader reader(work.path, work.path + ".idx");
                    auto docs = reader.read_json_lines_bytes_owned(work.start, work.end);
                    DFTRACER_UTILS_LOG_DEBUG("[Thread %zu] Loaded %zu documents from %s", std::hash<std::thread::id>{}(thread_id), docs.size(), work.path.c_str());
    ...
    for (size_t i = 0; i < load_indices.size(); ++i) {
        auto parse_task = Tasks::flatmap<json::OwnedJsonDocuments, Trace>(
            [](const json::OwnedJsonDocuments& documents) -> std::vector<Trace> {
