...
void TaskContext::schedule(TaskIndex task_id, std::any input, TaskIndex depends_on) {
    ...
    // All tasks (independent and dependent) should be submitted to the scheduler
    // The scheduler will handle dependency tracking internally  
    ...
    scheduler_->submit(task_id, task_ptr, std::move(input), completion_callback);
}
