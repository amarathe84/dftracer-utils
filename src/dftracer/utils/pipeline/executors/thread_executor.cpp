...
ThreadExecutor::ThreadExecutor(size_t max_threads)
    ...
    if (max_threads_ == 0) {
        max_threads_ = 2;  // Fallback to 2 threads if hardware_concurrency is
                           // not well-defined
    }
...
std::any ThreadExecutor::execute(const Pipeline& pipeline, std::any input,
    ...
    for (TaskIndex task_id : execution_order) {
        ...
        if (dependency_layers.size() <=
            static_cast<size_t>(task_levels[task_id])) {
    ...
    for (const auto& layer : dependency_layers) {
        ...
        for (size_t batch_start = 0; batch_start < layer.size();
             batch_start += max_threads_) {
            size_t batch_end =
                std::min(batch_start + max_threads_, layer.size());
                ...
                batch_futures[task_id] = std::async(
                    ...
                    [&pipeline, task_id, input, &task_outputs]() {
                        ...
                        if (pipeline.get_task_dependencies(task_id).empty()) {
                            task_input = input;
                        } else if (pipeline.get_task_dependencies(task_id)
                                       .size() == 1) {
