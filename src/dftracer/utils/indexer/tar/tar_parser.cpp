...
bool TarParser::parse_headers(const unsigned char* data, std::size_t data_size,
    ...
    while (offset + 512 <= data_size) {
        ...
        if (parse_single_header(header, stream_offset + offset, entry)) {
            ...
            // Skip to next header (current header + file data, rounded to 512 bytes)
...
bool TarParser::is_tar_header(const unsigned char* data, std::size_t data_size) {
    if (data_size < 512) {
        return false;
    }
    ...
    // Check for old tar format (no magic, but has valid checksum and reasonable values)
    if (header->name[0] != '\0' && TarParser::verify_checksum(header)) {
        // Additional sanity checks for old format
        std::uint64_t size = TarParser::parse_octal(header->size, sizeof(header->size));
...
bool TarParser::parse_single_header(const TarHeader* header,
    ...
    if (!TarParser::verify_checksum(header)) {
        DFTRACER_UTILS_LOG_DEBUG("TAR header checksum verification failed at offset %llu",
                                  header_offset);
        return false;
    }
...
bool TarParser::verify_checksum(const TarHeader* header) {
    ...
    for (std::size_t i = offsetof(TarHeader, checksum) + sizeof(header->checksum); 
         i < 512; i++) {
        calculated_sum += bytes[i];
    }
    
    // Parse stored checksum
    std::uint64_t stored_checksum = TarParser::parse_octal(header->checksum, sizeof(header->checksum));
...
std::string TarParser::extract_filename(const TarHeader* header) {
    ...
    if (header->prefix[0] != '\0') {
        // Use prefix + "/" + name
        std::size_t prefix_len = std::min(sizeof(header->prefix), 
                                          std::strlen(header->prefix));
    ...
    // Append main name
    std::size_t name_len = std::min(sizeof(header->name), 
