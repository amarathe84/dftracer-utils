...
bool query_tar_checkpoint(const SqliteDatabase &db, std::size_t target_offset,
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        checkpoint.checkpoint_idx = static_cast<std::uint64_t>(sqlite3_column_int64(stmt, 0));
        checkpoint.uc_offset = static_cast<std::uint64_t>(sqlite3_column_int64(stmt, 1));
        checkpoint.uc_size = static_cast<std::uint64_t>(sqlite3_column_int64(stmt, 2));
        checkpoint.c_offset = static_cast<std::uint64_t>(sqlite3_column_int64(stmt, 3));
        checkpoint.c_size = static_cast<std::uint64_t>(sqlite3_column_int64(stmt, 4));
        ...
        if (dict_data && dict_size > 0) {
            ...
            std::memcpy(checkpoint.dict_compressed.data(), dict_data, dict_size);
        }
        
        checkpoint.num_lines = static_cast<std::uint64_t>(sqlite3_column_int64(stmt, 7));
        checkpoint.first_line_num = static_cast<std::uint64_t>(sqlite3_column_int64(stmt, 8));
        checkpoint.last_line_num = static_cast<std::uint64_t>(sqlite3_column_int64(stmt, 9));
