...
TarIndexer::TarIndexer(const std::string &tar_gz_file_path, const std::string &index_path,
                       std::uint64_t checkpoint_size, bool rebuild_force)
    : tar_gz_path(tar_gz_file_path), idx_path(index_path), ckpt_size(checkpoint_size), 
      force_rebuild(rebuild_force), cached_is_valid(false), cached_archive_id(-1),
      cached_max_bytes(0), cached_num_lines(0), cached_num_files(0), 
...
TarIndexer::~TarIndexer() {
    try {
        DFTRACER_UTILS_LOG_DEBUG("Destroying TarIndexer for %s", tar_gz_path.c_str());
        if (db.is_open()) {
            close();
        }
        DFTRACER_UTILS_LOG_DEBUG("TarIndexer destruction completed for %s", tar_gz_path.c_str());
    } catch (const std::exception &e) {
        DFTRACER_UTILS_LOG_ERROR("Error during TarIndexer destruction: %s", e.what());
    } catch (...) {
...
void TarIndexer::open() {
    DFTRACER_UTILS_LOG_DEBUG("Opening TAR indexer database: %s", idx_path.c_str());
...
void TarIndexer::build() const {
    ...
    std::uint64_t final_ckpt_size = ckpt_size; // TODO: use determine_checkpoint_size like GZIP
    
    int file_id;
    insert_file_record(db, tar_gz_path_logical_path, bytes, mtime, hash, file_id);
    
    std::string archive_name = fs::path(tar_gz_path).filename().string();
    insert_archive_record(db, file_id, archive_name, 0, 0, archive_id); // Will update sizes later
...
bool TarIndexer::need_rebuild() const {
    ...
    try {
        ...
        if (query_stored_file_info(db, tar_gz_path_logical_path, stored_hash, stored_mtime)) {
            std::string current_hash = calculate_file_hash(tar_gz_path);
            time_t current_mtime = get_file_modification_time(tar_gz_path);
            
            return (stored_hash != current_hash || stored_mtime != current_mtime);
        }
    } catch (...) {
        return true;
    }
...
const std::string &TarIndexer::get_idx_path() const {
    return idx_path;
}

const std::string &TarIndexer::get_archive_path() const {
    return tar_gz_path;
}

const std::string &TarIndexer::get_tar_gz_path() const {
    return tar_gz_path;
}

std::string TarIndexer::get_format_name() const {
    return "TAR.GZ";
}

IndexerType TarIndexer::get_indexer_type() const {
    return IndexerType::TAR_GZ;
}
...
std::vector<IndexerCheckpoint> TarIndexer::get_checkpoints_for_line_range(
    ...
    return query_tar_checkpoints_for_line_range(db, archive_id, start_line, end_line);
}
...
std::vector<TarIndexer::TarFileInfo> TarIndexer::list_files() const {
    ...
    for (const auto &tf : tar_files) {
        result.emplace_back(TarFileInfo{
            tf.file_name,
            tf.file_size,
            tf.file_mtime,
            tf.typeflag,
...
bool TarIndexer::find_file(const std::string &file_name, TarFileInfo &file_info) const {
    int archive_id = get_archive_id();
    if (archive_id == -1) return false;
...
std::vector<TarIndexer::TarFileInfo> TarIndexer::find_files_in_range(std::uint64_t start_offset,
                                                                      std::uint64_t end_offset) const {
    int archive_id = get_archive_id();
    if (archive_id == -1) return {};
    
    auto tar_files = query_tar_files_in_range(db, archive_id, start_offset, end_offset);
    std::vector<TarFileInfo> result;
    result.reserve(tar_files.size());
...
static bool build_tar_index(const SqliteDatabase &db, int archive_id,
    ...
    while (true) {
        ...
        accumulated_data.insert(accumulated_data.end(),
                               inflater.out_buffer, inflater.out_buffer + result.bytes_read);
    ...
    if (!parser.parse_headers(accumulated_data.data(), accumulated_data.size(), 0, tar_entries)) {
        DFTRACER_UTILS_LOG_DEBUG("Failed to parse TAR headers from accumulated data", "");
    ...
    insert_archive_metadata_record(db, archive_id, ckpt_size, total_lines, total_uc_size);
