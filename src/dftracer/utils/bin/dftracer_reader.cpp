...
int main(int argc, char **argv) {
    ...
    program.add_description(
        "DFTracer utility for reading and indexing compressed files (GZIP, TAR.GZ)");
    program.add_argument("file").help("Compressed file to process (GZIP, TAR.GZ)").required();
    ...
    if (checkpoint_size <= 0) {
        DFTRACER_UTILS_LOG_ERROR(
            "%s", "Checkpoint size must be positive (greater than 0 and in MB)");
        return 1;
    }
    ...
    auto format = ReaderFactory::detect_format(gz_path);
    std::string idx_path = index_path.empty() ? 
        (gz_path + ReaderFactory::get_index_extension(format)) : 
        index_path;

    DFTRACER_UTILS_LOG_DEBUG("Detected format: %s", 
        format == gzip_common::ArchiveFormat::TAR_GZ ? "TAR.GZ" : 
        format == gzip_common::ArchiveFormat::GZIP ? "GZIP" : "UNKNOWN");
    ...
    try {
        ...
        indexer = IndexerFactory::create(gz_path, idx_path, checkpoint_size, force_rebuild);
        ...
        if (force_rebuild) {
            ...
            indexer = IndexerFactory::create(gz_path, idx_path, checkpoint_size, true);
    ...
    // read operations
    try {
        // Use ReaderFactory to create appropriate reader, transferring ownership of indexer
        auto reader = ReaderFactory::create(indexer.release());

        if (read_mode.find("bytes") == std::string::npos) {
            std::size_t start_line = (start == -1) ? 1 : static_cast<std::size_t>(start);
            ...
            std::size_t start_bytes_ = (start == -1) ? 0 : static_cast<std::size_t>(start);
