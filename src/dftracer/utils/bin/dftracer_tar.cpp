...
int main(int argc, char **argv) {
    ...
    argparse::ArgumentParser program("dftracer_tar", DFTRACER_UTILS_PACKAGE_VERSION);
    program.add_description("DFTracer utility for indexing and analyzing TAR.GZ archives");
    ...
    program.add_argument("-c", "--checkpoint-size")
        ...
        .scan<'d', std::size_t>()
        .default_value(static_cast<std::size_t>(Indexer::DEFAULT_CHECKPOINT_SIZE));
    ...
    DFTRACER_UTILS_LOG_DEBUG("Checkpoint size: %zu B (%zu MB)", 
                              checkpoint_size, checkpoint_size / (1024 * 1024));
    ...
    try {
        ...
        if (!indexer) {
            DFTRACER_UTILS_LOG_ERROR("Failed to create indexer for file '%s' - "
                                     "unsupported or unrecognized format", 
                                     archive_path.c_str());
            return 1;
        }

        DFTRACER_UTILS_LOG_INFO("Detected format: %s", indexer->get_format_name().c_str());
        DFTRACER_UTILS_LOG_INFO("Index file: %s", indexer->get_idx_path().c_str());
        ...
        if (show_info || (!list_files && !build_only)) {
            ...
            std::cout << "  Format: " << indexer->get_format_name() << std::endl;
            std::cout << "  Path: " << indexer->get_archive_path() << std::endl;
            std::cout << "  Index: " << indexer->get_idx_path() << std::endl;
            std::cout << "  Total size: " << indexer->get_max_bytes() << " bytes" << std::endl;
            std::cout << "  Total lines: " << indexer->get_num_lines() << std::endl;
            std::cout << "  Checkpoints: " << indexer->get_checkpoints().size() << std::endl;
        }
        ...
        if (list_files && indexer->get_format_name() == "TAR.GZ") {
            ...
            auto* tar_indexer = dynamic_cast<dftracer::utils::TarIndexer*>(indexer.get());
            if (tar_indexer) {
                auto files = tar_indexer->list_files();
                std::cout << "\nFiles in archive (" << files.size() << " total):" << std::endl;
                ...
                std::cout << "File listing not available for this format" << std::endl;
            }
        } else if (list_files) {
            std::cout << "File listing not available for GZIP format" << std::endl;
        }
