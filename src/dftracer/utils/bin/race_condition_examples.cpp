...
int main() {
    ...
    auto unsafe_task = [](int input, TaskContext& ctx) -> int {
        ...
        // ❌ RACE CONDITION: subtask might start before add_dependency is called!
    ...
    auto safe_current_task = [](int input, TaskContext& ctx) -> int {
        ...
        TaskIndex dependent = ctx.emit<int, int>(subtask, Input{input}, DependsOn{ctx.current()});
    ...
    auto independent_task = [](int input, TaskContext& ctx) -> int {
        ...
        DFTRACER_UTILS_LOG_INFO("INDEPENDENT: Emitted parallel task %d", parallel);
    ...
    auto complex_deps_task = [](int input, TaskContext& ctx) -> int {
        ...
        TaskIndex a_id = ctx.emit<int, int>(task_a, Input{input}, DependsOn{ctx.current()});  // A depends on current
        TaskIndex b_id = ctx.emit<int, int>(task_b, Input{input}, DependsOn{a_id});           // B depends on A
        TaskIndex c_id = ctx.emit<int, int>(task_c, Input{input}, DependsOn{b_id});           // C depends on B
    ...
    std::cout << "❌ ALSO NEVER: separate add_task() + add_dependency() calls\n";
