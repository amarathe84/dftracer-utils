...
int main() {
  DFTRACER_UTILS_LOGGER_INIT();
    ...
    // Example 2: Function task that emits another task (SAFE - using emit() with current())
    auto emitting_task = [](int input, TaskContext& ctx) -> int {
        DFTRACER_UTILS_LOG_INFO("Processing emitting task with input: %d", input);
        
        // SAFE: Emit a task that depends on current task atomically
        auto dependent_task = [](int x, TaskContext& ctx2) -> int {
            DFTRACER_UTILS_LOG_INFO("Processing dependent task with input: %d", x);
            return x + 100;
        };
        
        TaskIndex dependent_id = ctx.emit<int, int>(dependent_task, Input{input * 2}, DependsOn{ctx.current()});
        DFTRACER_UTILS_LOG_INFO("Emitted dependent task with ID: %d", dependent_id);
    ...
    TaskIndex task3 = string_pipeline.add_task<std::string, std::string>(string_task);
    DFTRACER_UTILS_LOG_INFO("Added string task with ID: %d", task3);
    
    // Example 4: Vector processing with dynamic task emission
    auto vector_processor = [](std::vector<int> input, TaskContext& ctx) -> int {
        DFTRACER_UTILS_LOG_INFO("Processing vector of size: %zu", input.size());
        
        int sum = 0;
        for (size_t i = 0; i < input.size(); ++i) {
            // SAFE: Emit child tasks that depend on current task
            auto element_processor = [i](int element, TaskContext& ctx2) -> int {
                DFTRACER_UTILS_LOG_INFO("Processing element %zu: %d", i, element);
                return element * element;
            };
            
            TaskIndex element_task = ctx.emit<int, int>(element_processor, Input{input[i]}, DependsOn{ctx.current()});
            DFTRACER_UTILS_LOG_INFO("Emitted element task %zu with ID: %d", i, element_task);
            sum += input[i];
        }
    ...
    TaskIndex task4 = vector_pipeline.add_task<std::vector<int>, int>(vector_processor);
    ...
    try {
        ...
        DFTRACER_UTILS_LOG_INFO("=== Executing int pipeline with integer input ===");
        ...
        DFTRACER_UTILS_LOG_INFO("=== Executing string pipeline with string input ===");
        ...
        DFTRACER_UTILS_LOG_INFO("=== Executing vector pipeline with vector input ===");
