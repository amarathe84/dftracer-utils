...
int main() {
    ...
    auto generator = [](int input, TaskContext& ctx) -> int {
        ...
        for (int i = 0; i < 5; ++i) {
            auto work_task = [i, input](int work_amount,
                                        TaskContext& ctx2) -> int {
                std::thread::id thread_id = std::this_thread::get_id();

                // Deterministic work - just math operations with no timing
                // dependencies
                int result = input;
                for (int j = 0; j < work_amount * 100; ++j) {
                    result = (result * 3 + 7) %
                             1000;  // Simple deterministic computation
                }
                ...
                DFTRACER_UTILS_LOG_INFO(
                    "Work task %d (amount=%d) executed by thread %zu, "
                    "result=%d",
    ...
    if (sequential_result == threaded_result &&
        threaded_result == threaded4_result) {
        std::cout
            << "✅ All results match! Pipeline execution is deterministic.\n";
        return 0;
    } else {
        std::cout << "❌ Results don't match! There may be non-deterministic "
                     "behavior.\n";
        return 1;
    }
}
