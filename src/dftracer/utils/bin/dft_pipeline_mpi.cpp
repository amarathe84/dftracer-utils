...
static void demonstrate_mpi_pipeline() {
    ...
    if (pipeline.is_master()) {
        ...
        std::cout << "Running distributed pipeline with " << pipeline.size() << " processes" << std::endl;
    }
    
    // Create multiple independent CPU-intensive tasks for distributed execution
    auto task1 = std::make_unique<MapTask<int, double>>(
        [](const int& x) { 
    ...
    auto task2 = std::make_unique<MapTask<int, double>>(
        [](const int& x) { 
    ...
    auto task3 = std::make_unique<MapTask<int, double>>(
        [](const int& x) { 
    ...
    auto task4 = std::make_unique<MapTask<int, double>>(
        [](const int& x) {
    ...
    auto task5 = std::make_unique<MapTask<int, double>>(
        [](const int& x) {
    ...
    if (pipeline.is_master()) {
        std::cout << "Input dataset: 6 integers with 5 independent heavy tasks" << std::endl;
    ...
    try {
        ...
        if (pipeline.is_master()) {
            ...
            std::cout << "MPI Pipeline completed in: " << duration.count() << " seconds" << std::endl;
            
            // Try to extract and display the final result
            try {
                auto final_result = std::any_cast<std::vector<double>>(result);
                std::cout << "Final result size: " << final_result.size() << std::endl;
                std::cout << "First few results: ";
                for (size_t i = 0; i < std::min(size_t(3), final_result.size()); ++i) {
                    std::cout << std::fixed << std::setprecision(2) << final_result[i] << " ";
                }
                std::cout << std::endl;
...
static void demonstrate_mpi_vs_sequential_comparison() {
    ...
    if (mpi_pipeline.is_master()) {
        ...
        std::cout << "Running identical workloads on sequential vs distributed pipeline" << std::endl;
    }
    
    // Create a CPU-intensive task factory function that can be reused
    auto create_cpu_intensive_task = [](int task_id) {
        return std::make_unique<MapTask<int, double>>(
            [task_id](const int& x) {
                double result = 0.0;
                int base_work = x * 2000000 + task_id * 500000;  // Extremely heavy computation
                
                if (task_id % 3 == 0) {
                    // Prime counting - limited to prevent overflow
                    int limit = std::min(base_work, 5000000); // Higher computation cap
                    for (int i = 2; i <= limit; ++i) {
                        bool is_prime = true;
                        for (int j = 2; j * j <= i && is_prime; ++j) {
                            if (i % j == 0) is_prime = false;
                    ...
                    int limit = std::min(base_work * 5, 10000000); // Much higher iteration cap
                    for (int i = 1; i <= limit; ++i) {
                        ...
                        if (i % 1000 == 0) {
                            if (!std::isfinite(result) || std::abs(result) > 1e6) {
                                result = std::copysign(1e6, result); // Clamp to safe range
                            }
                        }
                    }
                } else {
                    ...
                    for (int i = 0; i < size; ++i) {
                        for (int j = 0; j < size; ++j) {
                            ...
                            if (std::isfinite(val)) {
                                result += val * val;
                                double sqrt_val = std::sqrt(std::abs(val * i * j + 1));
                                if (std::isfinite(sqrt_val)) {
                                    result += sqrt_val;
                                }
                                
                                // Safer power computation
                                for (int k = 0; k < 3; ++k) { // Reduced from 5 to 3
                                    double power = 1.0 + k * 0.1;
                                    if (std::abs(val) < 10.0) { // Much tighter bound
                                        double pow_val = std::pow(std::abs(val), power);
    ...
    std::vector<int> input = {25, 30, 35}; // Even larger input for extreme work
    int num_tasks = mpi_pipeline.size() * 4; // 4 tasks per process for maximum load
    
    if (mpi_pipeline.is_master()) {
        std::cout << "Input: " << input.size() << " integers" << std::endl;
        std::cout << "Tasks: " << num_tasks << " CPU-intensive independent tasks" << std::endl;
    ...
    if (mpi_pipeline.is_master()) {
        ...
        sequential_time = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
        ...
        try {
            ...
            std::cout << "Sequential result size: " << sequential_result.size() << std::endl;
            std::cout << "Sequential first few values: ";
            for (size_t i = 0; i < std::min(size_t(3), sequential_result.size()); ++i) {
                std::cout << std::fixed << std::setprecision(2) << sequential_result[i] << " ";
            }
            std::cout << std::endl;
        } catch (const std::bad_any_cast& e) {
            std::cout << "Sequential result type: " << seq_result.type().name() << std::endl;
        }
        std::cout << std::endl;
    }
    ...
    try {
        ...
        mpi_time = std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count();
        ...
        if (mpi_pipeline.is_master()) {
            ...
            try {
                ...
                std::cout << "MPI result size: " << mpi_result.size() << std::endl;
                std::cout << "MPI first few values: ";
                for (size_t i = 0; i < std::min(size_t(3), mpi_result.size()); ++i) {
                    std::cout << std::fixed << std::setprecision(2) << mpi_result[i] << " ";
                }
                std::cout << std::endl;
            } catch (const std::bad_any_cast& e) {
                std::cout << "MPI result type: " << mpi_result_any.type().name() << std::endl;
                // Try other possible types
                try {
                    auto int_result = std::any_cast<std::vector<int>>(mpi_result_any);
                    std::cout << "MPI result (as int vector) size: " << int_result.size() << std::endl;
                    std::cout << "MPI int values: ";
                    for (size_t i = 0; i < std::min(size_t(3), int_result.size()); ++i) {
                        std::cout << int_result[i] << " ";
                    }
                    std::cout << std::endl;
                } catch (const std::bad_any_cast&) {
                    try {
                        double single_result = std::any_cast<double>(mpi_result_any);
                        std::cout << "MPI result (single double): " << single_result << std::endl;
                        mpi_result = {single_result}; // Convert to vector for comparison
                    } catch (const std::bad_any_cast&) {
                        std::cout << "Could not cast MPI result to any expected type" << std::endl;
                    }
                }
            }
            std::cout << std::endl;
            ...
            std::cout << "MPI (" << mpi_pipeline.size() << " processes): " << mpi_time << " ms" << std::endl;
            
            if (mpi_time > 0) {
                double speedup = static_cast<double>(sequential_time) / mpi_time;
                ...
                std::cout << "Time saved: " << (sequential_time - mpi_time) << " ms" << std::endl;
                ...
                if (!sequential_result.empty() && !mpi_result.empty()) {
                    if (sequential_result.size() == mpi_result.size()) {
                        std::cout << "Result sizes match: " << sequential_result.size() << std::endl;
                        
                        // Check if results are approximately equal (allowing for floating-point differences)
                        bool results_match = true;
                        double max_diff = 0.0;
                        double tolerance = 1e-6; // Allow small floating-point differences
                        
                        for (size_t i = 0; i < sequential_result.size() && i < 10; ++i) { // Check first 10 elements
                            double diff = std::abs(sequential_result[i] - mpi_result[i]);
                        ...
                        if (results_match) {
                            std::cout << "✓ Results match (within tolerance " << tolerance << ")" << std::endl;
                            std::cout << "Max difference: " << std::scientific << max_diff << std::endl;
                        } else {
                            std::cout << "✗ Results differ beyond tolerance" << std::endl;
                            std::cout << "Max difference: " << std::scientific << max_diff << std::endl;
                            std::cout << "Sample differences:" << std::endl;
                            for (size_t i = 0; i < std::min(size_t(3), sequential_result.size()); ++i) {
                                std::cout << "  [" << i << "] Seq: " << sequential_result[i] 
                                          << " MPI: " << mpi_result[i] 
                                          << " Diff: " << (sequential_result[i] - mpi_result[i]) << std::endl;
                            }
                        }
                    } else {
                        std::cout << "✗ Result sizes differ: Sequential=" << sequential_result.size() 
                                  << " MPI=" << mpi_result.size() << std::endl;
                    }
                } else {
                    std::cout << "Unable to compare results (empty or wrong type)" << std::endl;
                }
            } else {
