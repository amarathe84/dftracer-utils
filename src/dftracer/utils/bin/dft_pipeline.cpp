...
static void demonstrate_sequential_vs_thread_comparison() {
    std::cout << "=== Sequential vs Thread Comparison (Parallel DAG) ===" << std::endl;
    
    // Create multiple independent VERY CPU-intensive tasks that can run in parallel
    auto create_parallel_tasks = []() {
        ...
        // Task 2: Heavy mathematical series (trigonometry + matrix-like operations)
        auto task2 = std::make_unique<MapTask<int, double>>(
            [](const int& x) { 
                ...
                for (int i = 1; i <= iterations; ++i) {
                    ...
                    result += std::sin(angle) * std::cos(angle * 2) * std::tan(angle * 0.5);
        ...
        // Task 3: Heavy combinatorial computation (fibonacci-like recursive work)
        ...
        return std::make_tuple(std::move(task1), std::move(task2), std::move(task3), 
                             std::move(task4), std::move(task5));
    };
    
    // Smaller dataset but extremely heavy computation per task
    std::vector<int> input;
    for (int i = 10; i <= 15; ++i) {  // Only 6 elements, but massive computation
        input.push_back(i);
    }
    std::cout << "Dataset: 6 integers with 5 independent VERY heavy parallel tasks (seconds-level work)" << std::endl;
    ...
    // Sequential Pipeline - tasks run one after another
    {
        ...
        std::cout << "Running sequential pipeline (this will take a while)..." << std::endl;
    ...
    // Thread Pipeline - tasks can run in parallel
    {
        ...
        std::cout << "Running thread pipeline (should be much faster)..." << std::endl;
        auto start = std::chrono::high_resolution_clock::now();
        std::any result = thread_pipeline.execute(input);
        auto end = std::chrono::high_resolution_clock::now();
        
        auto duration = std::chrono::duration_cast<std::chrono::seconds>(end - start);
        std::cout << "Thread Time: " << duration.count() << " seconds" << std::endl;
    }
...
static void demonstrate_complex_dag() {
    ...
    auto filter_task = std::make_unique<FilterTask<int>>(
    ...
    // Note: This is a simplified example - real DAG merging would need more complex logic
    auto sum_task = std::make_unique<SumTask<double>>(
        std::plus<double>{}, 0.0
    );
    ...
    // Note: In a real implementation, you'd need a merge task for multiple inputs
