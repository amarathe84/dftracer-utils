...
int main() {
    ...
    auto heavy_task_generator = [](int base_input, TaskContext& ctx) -> int {
        std::this_thread::sleep_for(std::chrono::milliseconds(10)); // Some initial work
        
        DFTRACER_UTILS_LOG_INFO("Generator task processing input: %d", base_input);
        ...
        for (int i = 0; i < 5; ++i) {
            ...
            auto work_task = [i, base_input](int work_amount, TaskContext& ctx2) -> int {
                std::thread::id thread_id = std::this_thread::get_id();
                auto start = std::chrono::high_resolution_clock::now();
                
                // Simulate work with varying intensity
                std::this_thread::sleep_for(std::chrono::milliseconds(work_amount * 10));
                ...
                std::cout << "COUNTER: Work task executed, tasks_executed=" << tasks_executed << ", work_amount=" << work_amount << std::endl;
                
                // Log which thread executed this task
                std::hash<std::thread::id> hasher;
                size_t thread_hash = hasher(thread_id) % 1000;
                
                DFTRACER_UTILS_LOG_INFO("Work task %d (amount=%d) executed by thread %zu in %ldms", 
    ...
    auto secondary_generator = [](int input, TaskContext& ctx) -> int {
        ...
        for (int i = 0; i < 3; ++i) {
            auto heavy_work = [i, input](int load, TaskContext& ctx2) -> int {
                ...
                std::this_thread::sleep_for(std::chrono::milliseconds(load * 15));
                ...
                std::cout << "COUNTER: Heavy task executed, tasks_executed=" << tasks_executed << ", load=" << load << std::endl;
                
                std::hash<std::thread::id> hasher;
                size_t thread_hash = hasher(thread_id) % 1000;
                
                DFTRACER_UTILS_LOG_INFO("Heavy task %d (load=%d) executed by thread %zu in %ldms", 
            ...
            DFTRACER_UTILS_LOG_INFO("Emitted heavy task %d with load %d (ID: %d)", i, load, heavy_id);
        }
    ...
    for (int num_threads : thread_counts) {
        ...
        // Add tasks to pipeline with dependencies to create initial work distribution
        ...
        try {
            ...
            std::cout << "Average work per task: " << (tasks_executed > 0 ? total_work_items.load() / tasks_executed.load() : 0) << "\n";
            std::cout << "Throughput: " << (total_duration.count() > 0 ? (tasks_executed.load() * 1000) / total_duration.count() : 0) << " tasks/sec\n";
            
        } catch (const std::exception& e) {
            DFTRACER_UTILS_LOG_ERROR("Execution with %d threads failed: %s", num_threads, e.what());
        }
    }
    ...
    TaskIndex seq_gen1 = sequential_pipeline.add_task<int, int>(heavy_task_generator);
    TaskIndex seq_gen2 = sequential_pipeline.add_task<int, int>(secondary_generator);
    ...
    try {
        ...
        std::cout << "Sequential execution time: " << total_duration.count() << "ms\n";
    ...
    std::cout << "✅ Benefits: Load balancing, scalability, reduced contention\n";
