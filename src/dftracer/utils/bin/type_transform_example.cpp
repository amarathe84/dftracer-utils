...
int main() {
    ...
    auto string_to_vector = [](std::string input, TaskContext& ctx) -> std::vector<char> {
        DFTRACER_UTILS_LOG_INFO("Converting string '%s' to vector<char>", input.c_str());
        std::vector<char> result(input.begin(), input.end());
        return result;
    };
    
    TaskIndex t1 = int_to_string_pipeline.add_task<int, std::string>(int_to_string);
    TaskIndex t2 = int_to_string_pipeline.add_task<std::string, std::vector<char>>(string_to_vector);
    ...
    try {
        ...
        std::vector<char> final_result = std::any_cast<std::vector<char>>(result);
    ...
    auto vector_to_average = [](std::vector<int> input, TaskContext& ctx) -> double {
        DFTRACER_UTILS_LOG_INFO("Calculating average of vector with %zu elements", input.size());
    ...
    auto double_to_formatted_string = [](double input, TaskContext& ctx) -> std::string {
    ...
    TaskIndex t3 = vector_to_string_pipeline.add_task<std::vector<int>, double>(vector_to_average);
    TaskIndex t4 = vector_to_string_pipeline.add_task<double, std::string>(double_to_formatted_string);
    ...
    try {
        ...
        std::any result = executor.execute(vector_to_string_pipeline, input_data);
    ...
    auto multi_transform_task = [](std::string input, TaskContext& ctx) -> int {
        DFTRACER_UTILS_LOG_INFO("Processing string '%s' for multi-transform", input.c_str());
        ...
        TaskIndex length_task = ctx.emit<std::string, int>(get_length, Input{input});
        
        // Emit task 2: string → double (hash as double)
        auto string_to_hash = [](std::string s, TaskContext& ctx2) -> double {
            DFTRACER_UTILS_LOG_INFO("Computing hash for string '%s'", s.c_str());
            std::hash<std::string> hasher;
            size_t hash_val = hasher(s);
            return static_cast<double>(hash_val % 1000) / 100.0;  // Normalize to 0-10 range
        };
        TaskIndex hash_task = ctx.emit<std::string, double>(string_to_hash, Input{input});
        
        DFTRACER_UTILS_LOG_INFO("Emitted length_task=%d, hash_task=%d", length_task, hash_task);
        
        // Return the number of characters processed
        return static_cast<int>(input.size());
    };
    
    TaskIndex t5 = dynamic_transform_pipeline.add_task<std::string, int>(multi_transform_task);
    ...
    try {
        ...
        std::any result = executor.execute(dynamic_transform_pipeline, test_string);
    ...
    std::cout << "2. vector<int> → double → string: Statistics with formatting\n"; 
    std::cout << "3. Dynamic emission: string → (int + double): Multi-output processing\n";
