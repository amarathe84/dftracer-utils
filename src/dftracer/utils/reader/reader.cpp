   ...
   public:
    class InflationSession {
       public:
        ...
        int read(unsigned char *output, size_t output_size,
                 size_t *bytes_read) {
   ...
   protected:
    ...
    bool try_initialize_with_checkpoint(
        ...
        if (should_use_first_checkpoint) {
            if (indexer.find_checkpoint(0, *checkpoint_)) {
                if (inflate_init_from_checkpoint(inflate_state_.get(),
                                                 file_handle_,
                                                 checkpoint_.get()) == 0) {
                    DFTRACER_UTILS_LOG_DEBUG(
            ...
            if (indexer.find_checkpoint(start_bytes, *checkpoint_)) {
                if (inflate_init_from_checkpoint(inflate_state_.get(),
                                                 ...
                                                 checkpoint_.get()) == 0) {
                    DFTRACER_UTILS_LOG_DEBUG(
                        "Using checkpoint at uncompressed offset %zu for "
                        "target %zu",
                        checkpoint_->uc_offset, start_bytes);
                    return true;
                }
    ...
    int inflate_init_from_checkpoint(
        ...
        if (checkpoint->bits != 0) {
            ...
            if (inflatePrime(&state->zs, checkpoint->bits, prime_value) !=
                Z_OK) {
                DFTRACER_UTILS_LOG_ERROR(
    ...
    int decompress_window(const unsigned char *compressed,
                          size_t compressed_size, unsigned char *window,
...
class LineByteStreamingSession : public BaseStreamingSession {
   ...
   public:
    LineByteStreamingSession()
        : BaseStreamingSession(), actual_start_bytes_(0) {
    ...
    void initialize(const std::string &gz_path, size_t start_bytes,
        ...
        DFTRACER_UTILS_LOG_DEBUG(
            "JSON streaming session initialized: actual_start=%zu, "
            "target_end=%zu",
...
// Prefetch buffer for writing
   ...
   private:
    ...
    size_t apply_range_and_boundary_limits(char *buffer,
   ...
   public:
    Impl(const std::string &gz_path, const std::string &idx_path,
        ...
        try {
            ...
            session_factory_ =
                std::make_unique<StreamingSessionFactory>(*indexer_);
    ...
    size_t read(size_t start_bytes, size_t end_bytes, char *buffer,
        ...
        if (session_factory_->needs_new_raw_session(
                byte_session_.get(), gz_path_, start_bytes, end_bytes)) {
    ...
    dftracer::utils::json::OwnedJsonDocuments read_json_lines_owned(
        ...
        std::string lines_data = read_lines(start, end);
        return dftracer::utils::json::parse_json_lines_owned(lines_data.data(),
                                                             lines_data.size());
    }
    ...
    dftracer::utils::json::JsonDocuments read_json_lines_bytes(
        ...
        ErrorHandler::validate_parameters(buffer.get(), default_buffer_size_,
                                          start_bytes, end_bytes,
        ...
        while (
            (bytes_read = read_line_bytes(start_bytes, end_bytes, buffer.get(),
    ...
    dftracer::utils::json::OwnedJsonDocuments read_json_lines_bytes_owned(
        ...
        return dftracer::utils::json::parse_json_lines_owned(
            buffer_content.data(), buffer_content.size());
    }
    ...
    void set_buffer_size(size_t size) { default_buffer_size_ = size; }
...
dftracer::utils::json::JsonDocuments Reader::read_json_lines(size_t start,
                                                             size_t end) {
...
void Reader::set_buffer_size(size_t size) { pImpl_->set_buffer_size(size); }
