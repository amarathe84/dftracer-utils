...
class ByteStream : public Stream {
   public:
    ...
    virtual void initialize(const std::string &gz_path, std::size_t start_bytes,
        ...
        DFTRACER_UTILS_LOG_DEBUG("ByteStream::initialize - start_bytes=%zu, end_bytes=%zu", start_bytes, end_bytes);
        ...
        DFTRACER_UTILS_LOG_DEBUG("ByteStream::initialize - checkpoint uc_offset=%zu, using_checkpoint=%s", 
                                 current_pos, use_checkpoint_ ? "true" : "false");
        if (start_bytes > current_pos) {
            DFTRACER_UTILS_LOG_DEBUG("ByteStream::initialize - skipping %zu bytes to reach start_bytes", start_bytes - current_pos);
            skip(start_bytes);
        }
        DFTRACER_UTILS_LOG_DEBUG("ByteStream::initialize - completed, current_position_=%zu", current_position_);
    }
    ...
    virtual std::size_t stream(char *buffer, std::size_t buffer_size) override {
        ...
        DFTRACER_UTILS_LOG_DEBUG("ByteStream::stream - read result: result=%d, bytes_read=%zu", result, bytes_read);
        if (!result || bytes_read == 0) {
            DFTRACER_UTILS_LOG_DEBUG("ByteStream::stream - marking as finished due to read failure or 0 bytes");
            is_finished_ = true;
            return 0;
        }
