...
/**
 * TAR-aware line byte stream that provides logical line-based reading from TAR.GZ files.
 * Presents concatenated file contents without TAR headers, ensuring complete lines.
 */
class TarLineByteStream : public TarStream {
   private:
    ...
    std::string line_buffer_;  // Buffer for accumulating partial lines across files
   ...
   public:
    ...
    std::size_t stream(char *buffer, std::size_t buffer_size) override {
        ...
        if (!line_buffer_.empty()) {
            std::size_t buffer_bytes = std::min(line_buffer_.size(), remaining_buffer);
        ...
        while (remaining_buffer > 0 && current_position_ < target_end_bytes_ && current_file_) {
            // Calculate how much we can read from current file
            std::size_t file_remaining = current_file_->file_size - current_file_offset_;
            std::size_t logical_remaining = target_end_bytes_ - current_position_;
            std::size_t max_read = std::min(remaining_buffer, std::min(file_remaining, logical_remaining));
            ...
            std::size_t bytes_read = current_file_stream_->stream(temp_buffer.data(), max_read);
            
            DFTRACER_UTILS_LOG_DEBUG(
                "TarLineByteStream::stream - read %zu bytes from file %s (offset %zu)",
                bytes_read, current_file_->file_name.c_str(), current_file_offset_);
            ...
            std::size_t processed = process_line_data(
                ...
                buffer + total_bytes_written, remaining_buffer);
            ...
            // If we couldn't process all data, we're likely at a buffer boundary
            if (processed == 0 && bytes_read > 0) {
                break;
            }
        }
        
        if (total_bytes_written == 0 || current_position_ >= target_end_bytes_) {
            is_finished_ = true;
        }
        
        DFTRACER_UTILS_LOG_DEBUG(
            "TarLineByteStream::stream - total bytes written: %zu (position: %zu / %zu)",
   ...
   private:
    void initialize_current_file_stream() {
        ...
        if (!tar_indexer_ptr_->find_file(current_file_->file_name, tar_file_info)) {
        ...
        std::uint64_t actual_start = tar_file_info.data_offset + current_file_offset_;
        std::uint64_t actual_end = tar_file_info.data_offset + current_file_->file_size;
        
        // Clamp to our target range
        std::uint64_t logical_start = current_file_->logical_start_offset + current_file_offset_;
        std::uint64_t logical_end = std::min(
            static_cast<std::uint64_t>(target_end_bytes_),
        ...
        DFTRACER_UTILS_LOG_DEBUG(
            "Initializing line stream for file %s: actual[%lu-%lu] logical[%lu-%lu]",
        ...
        current_file_stream_->initialize(
            current_gz_path_, 
            static_cast<std::size_t>(actual_start),
            static_cast<std::size_t>(actual_end),
            *tar_indexer_ptr_);
    }
     ...
     */
    std::size_t process_line_data(const char* input_data, std::size_t input_size,
                                  char* output_buffer, std::size_t output_size) {
        ...
        while (search_pos < working_buffer.size() && bytes_written < output_size) {
            ...
            std::memcpy(output_buffer + bytes_written, 
                       working_buffer.c_str() + search_pos, 
                       line_size);
