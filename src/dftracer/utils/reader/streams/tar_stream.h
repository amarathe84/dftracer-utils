   ...
   protected:
    void build_logical_mapping(TarIndexer &tar_indexer) {
        ...
        for (const auto& tar_file : tar_files) {
            ...
            mapped_file.logical_end_offset = logical_offset + tar_file.file_size;
            ...
            DFTRACER_UTILS_LOG_DEBUG(
                ...
                mapped_file.file_name.c_str(),
                mapped_file.logical_start_offset,
                mapped_file.logical_end_offset,
                mapped_file.file_size);
        }
        
        logical_mapping_initialized_ = true;
        DFTRACER_UTILS_LOG_DEBUG("TAR logical mapping complete: %zu files", file_mapping_.size());
    }
    ...
    void seek_to_logical_position(std::size_t logical_pos) {
        ...
        for (std::size_t i = 0; i < file_mapping_.size(); ++i) {
            ...
            if (logical_pos >= file.logical_start_offset && logical_pos < file.logical_end_offset) {
                current_file_ = &file;
        ...
        if (!current_file_) {
            ...
            current_file_offset_ = 0;
            DFTRACER_UTILS_LOG_DEBUG("Positioned beyond all files (logical pos %zu)", logical_pos);
        }
    }
    ...
    bool advance_to_next_file() {
        ...
        DFTRACER_UTILS_LOG_DEBUG(
            "Advanced to file %s", current_file_->file_name.c_str());
        return true;
    }
    ...
    std::uint64_t get_current_file_data_offset() const {
        ...
        return 0; // This will be filled by subclasses that have access to the indexer
    }
};
