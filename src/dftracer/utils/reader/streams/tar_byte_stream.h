...
/**
 * TAR-aware byte stream that provides logical content reading from TAR.GZ files.
 * Presents concatenated file contents without TAR headers.
 */
   ...
   public:
    ...
    std::size_t stream(char *buffer, std::size_t buffer_size) override {
        ...
        while (remaining_buffer > 0 && current_position_ < target_end_bytes_ && current_file_) {
            ...
            DFTRACER_UTILS_LOG_DEBUG(
                "TarByteStream::stream - read %zu bytes from file %s (offset %zu)",
        ...
        DFTRACER_UTILS_LOG_DEBUG(
            "TarByteStream::stream - total bytes read: %zu (position: %zu / %zu)",
