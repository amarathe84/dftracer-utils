 ...
 * 1. Archive-level access: Concatenated view of all file contents (without TAR headers)
...
class TarReader : public Reader {
   public:
    ...
    struct TarFileInfo {
        ...
        char typeflag;
        std::uint64_t logical_start_offset;  // Offset in concatenated logical view
        std::uint64_t logical_end_offset;    // End offset in concatenated logical view
        std::uint64_t logical_start_line;    // Line number in concatenated logical view
        std::uint64_t logical_end_line;      // End line number in concatenated logical view
        std::uint64_t estimated_lines;       // Estimated number of lines in this file
    };
    ...
    // BaseReader interface - operates on concatenated logical view (all file contents)
    ...
    std::string read_lines(std::size_t start_line, std::size_t end_line) override;
