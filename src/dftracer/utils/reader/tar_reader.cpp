...
TarReader::TarReader(const std::string &tar_gz_path_, const std::string &idx_path_,
    ...
    try {
        indexer = std::make_unique<TarIndexer>(tar_gz_path, idx_path, index_ckpt_size, false);
        is_open = true;
        ...
        throw std::runtime_error("Failed to initialize TAR reader with indexer: " + std::string(e.what()));
    }
}
...
TarReader::~TarReader() {
    try {
        DFTRACER_UTILS_LOG_DEBUG("Destroying TarReader for %s", tar_gz_path.c_str());
        
        if (indexer) {
            if (!is_indexer_initialized_internally) {
                // For externally managed indexer, release ownership without deleting
                DFTRACER_UTILS_LOG_DEBUG("Releasing externally managed TAR indexer");
                indexer.release();
            } else {
                // For internally managed indexer, let unique_ptr destructor handle it
                DFTRACER_UTILS_LOG_DEBUG("Destroying internally managed TAR indexer");
            }
        }
        
        DFTRACER_UTILS_LOG_DEBUG("TarReader destruction completed for %s", tar_gz_path.c_str());
    } catch (const std::exception &e) {
        DFTRACER_UTILS_LOG_ERROR("Error during TarReader destruction: %s", e.what());
    }
}
...
TarReader::TarReader(TarReader &&other) noexcept 
    : tar_gz_path(std::move(other.tar_gz_path)),
      ...
      is_indexer_initialized_internally(other.is_indexer_initialized_internally),
...
std::string TarReader::get_format_name() const {
    return "TAR.GZ";
}

const std::string &TarReader::get_archive_path() const {
    return tar_gz_path;
}

const std::string &TarReader::get_idx_path() const {
    return idx_path;
}
...
std::size_t TarReader::read_line_bytes(std::size_t start_bytes, std::size_t end_bytes,
                                      char *buffer, std::size_t buffer_size) {
    build_logical_mapping();
    ...
    std::size_t actual_end = std::min(end_bytes, static_cast<std::size_t>(cached_total_logical_bytes));
    ...
    if (start_bytes > 0) {
        ...
        std::size_t lookback_start = (start_bytes >= 512) ? start_bytes - 512 : 0;
    ...
    while (search_pos < bytes_read && output_pos < buffer_size) {
        ...
        if (newline_pos < bytes_read) {
            ...
            if (output_pos + line_len <= buffer_size) {
                std::memcpy(buffer + output_pos, temp_buffer.data() + search_pos, line_len);
                output_pos += line_len;
...
std::string TarReader::read_lines(std::size_t start_line, std::size_t end_line) {
    build_logical_mapping();
    ...
    std::size_t actual_end_line = std::min(end_line, static_cast<std::size_t>(cached_total_logical_lines));
    ...
    for (const auto &file_info : cached_file_mapping) {
        ...
        std::size_t file_start_line = std::max(start_line, current_global_line) - current_global_line + 1;
        std::size_t file_end_line = std::min(actual_end_line, static_cast<std::size_t>(current_global_line + file_info.estimated_lines - 1)) - current_global_line + 1;
        
        std::string file_content = read_file_content(file_info, 0, file_info.file_size);
        result += extract_lines_from_content(file_content, file_start_line, file_end_line);
...
void TarReader::read_lines_with_processor(std::size_t start_line, std::size_t end_line,
    ...
    for (const auto &file_info : cached_file_mapping) {
        ...
        // Read content from this file
        std::string file_content = read_file_content(file_info, 0, file_info.file_size);
        process_content_lines(file_content, processor, current_global_line, start_line, actual_end_line);
...
void TarReader::read_line_bytes_with_processor(std::size_t start_bytes,
    ...
    for (const auto &file_info : cached_file_mapping) {
        ...
        std::size_t file_start = (start_bytes > file_info.logical_start_offset) 
            ? start_bytes - file_info.logical_start_offset : 0;
        std::size_t file_end = std::min(file_info.file_size, 
            actual_end - file_info.logical_start_offset);
        
        if (file_start < file_end) {
            std::string file_content = read_file_content(file_info, file_start, file_end - file_start);
...
bool TarReader::is_valid() const {
    return is_open && indexer;
}

// File-level operations - NOT YET IMPLEMENTED 
std::string TarReader::read_file(const std::string &filename, std::size_t start_line, std::size_t end_line) {
    (void)filename; (void)start_line; (void)end_line; // Suppress unused warnings
    return ""; // Placeholder
}

bool TarReader::find_file(const std::string &filename, TarFileInfo &file_info) const {
    build_logical_mapping();
    
    auto it = std::find_if(cached_file_mapping.begin(), cached_file_mapping.end(),
...
void TarReader::build_logical_mapping() const {
    ...
    DFTRACER_UTILS_LOG_DEBUG("Building logical content mapping for TAR.GZ file");
    ...
    try {
        ...
        for (const auto &tar_file : tar_files) {
            ...
            file_info.estimated_lines = tar_file.file_size > 0 ? (tar_file.file_size / 50) + 1 : 1;
            file_info.logical_end_line = logical_line + file_info.estimated_lines - 1;
        ...
        DFTRACER_UTILS_LOG_DEBUG("Built logical mapping: %zu files, %zu bytes, %zu lines",
                                 cached_file_mapping.size(), 
                                 cached_total_logical_bytes,
                                 cached_total_logical_lines);
    } catch (const std::exception &e) {
        throw std::runtime_error("Failed to build TAR logical mapping: " + std::string(e.what()));
    }
}

std::size_t TarReader::read_logical(std::size_t start_bytes, std::size_t end_bytes, 
    ...
    DFTRACER_UTILS_LOG_DEBUG("TAR logical read: start=%zu, end=%zu, buffer_size=%zu",
                             start_bytes, actual_end, buffer_size);
    ...
    for (const auto &file_info : cached_file_mapping) {
        ...
        if (file_start < file_end) {
            std::size_t bytes_to_read = std::min(file_end - file_start, buffer_size - bytes_written);
            std::string file_content = read_file_content(file_info, file_start, bytes_to_read);
            
            std::memcpy(buffer + bytes_written, file_content.c_str(), file_content.length());
            bytes_written += file_content.length();
        }
    }
    
    DFTRACER_UTILS_LOG_DEBUG("TAR logical read: returning %zu bytes", bytes_written);
...
std::string TarReader::read_file_content(const TarFileInfo &file_info, 
    ...
    try {
        // Calculate the absolute offset in the logical stream where the file data starts
        ...
        std::size_t max_end = file_info.logical_start_offset + file_info.file_size;
        ...
        auto byte_stream = tar_stream_factory.create_byte_stream(tar_gz_path, file_data_start, file_data_end);
        
        if (!byte_stream) {
            DFTRACER_UTILS_LOG_DEBUG("Failed to create TAR byte stream for file content");
            return "";
        }
        
        std::size_t total_read = 0;
        while (total_read < actual_size) {
            std::size_t chunk_size = std::min(actual_size - total_read, static_cast<std::size_t>(8192));
            std::size_t bytes_read = byte_stream->stream(buffer.data() + total_read, chunk_size);
        ...
        DFTRACER_UTILS_LOG_DEBUG("Error reading TAR file content: %s", e.what());
        return "";
    }
}
