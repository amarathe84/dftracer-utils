...
bool MPIFilteredTraceReader::read(const std::string& trace_file, internal::CallTree& graph) {
...
MPICallGraphBuilder::MPICallGraphBuilder(const MPICallGraphConfig& config)
    : config_(config)
    , call_graph_(std::make_unique<internal::CallTree>())
    , rank_(0)
    , world_size_(1)
...
void MPICallGraphBuilder::merge_from_serializable(const SerializableProcessGraph& serializable) {
    internal::ProcessCallTree& graph = (*call_graph_)[serializable.key];
...
std::unique_ptr<internal::CallTree> MPICallGraphBuilder::load(const std::string& filename) {
