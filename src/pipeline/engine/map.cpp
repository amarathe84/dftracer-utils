...
static inline std::size_t normalize_stride(std::size_t stride, std::size_t elem_size) {
  return stride == 0 ? elem_size : stride;
}

void run_map(ExecutionContext& ctx,
  ...
  if ((in.count > 0) && (!in.data || !out.data)) {
    throw std::invalid_argument("run_map: null data pointer with non-zero count");
  }
  if (!op.fn && !op.fn_with_state) {
    throw std::invalid_argument("run_map: null map function");
  }

  const std::size_t in_stride  = normalize_stride(in.stride,  in.elem_size);
  const std::size_t out_stride = normalize_stride(out.stride, out.elem_size);
