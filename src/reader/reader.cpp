...
class ReaderError : public std::runtime_error
{
  public:
    ...
    ReaderError(Type type, const std::string &message) : std::runtime_error(format_message(type, message)), type_(type)
    {
    }
...
class CompressionManager
{
  public:
    ...
    {
      public:
        explicit InflationSession(InflateState *state, bool owns = false) : state_(state), owns_state_(owns) {}
...
static void validate_read_parameters(const char *buffer, size_t buffer_size, size_t start_bytes, size_t end_bytes)
{
...
class BaseStreamingSession
{
  ...
  public:
    BaseStreamingSession()
        : start_bytes_(0), current_position_(0), target_end_bytes_(0), is_active_(false), is_finished_(false),
          file_handle_(nullptr), decompression_initialized_(false)
    {
    }
    ...
    {
        return current_gz_path_ == gz_path && start_bytes_ == start_bytes && target_end_bytes_ == end_bytes;
    }
    ...
    virtual void
    initialize(const std::string &gz_path, size_t start_bytes, size_t end_bytes, dft::indexer::Indexer &indexer) = 0;
  ...
  protected:
    ...
    void initialize_compression(const std::string &gz_path, size_t start_bytes, dft::indexer::Indexer &indexer)
    {
...
class JsonStreamingSession : public BaseStreamingSession
{
  ...
  public:
    ...
    void initialize(const std::string &gz_path,
                    size_t start_bytes,
                    size_t end_bytes,
                    dft::indexer::Indexer &indexer) override
    {
        spdlog::debug(
            "Initializing JSON streaming session for range [{}, {}] from {}", start_bytes, end_bytes, gz_path);
        ...
        spdlog::debug(
            "JSON streaming session initialized: actual_start={}, target_end={}", actual_start_bytes_, end_bytes);
    }
    ...
    size_t stream_chunk(char *buffer, size_t buffer_size) override
    {
        ...
        {
            throw ReaderError(ReaderError::INITIALIZATION_ERROR, "Streaming session not properly initialized");
        }
        ...
        while (attempts < max_attempts)
        {
            ...
            if (bytes_to_copy > 0)
            {
                ...
                {
                    memmove(incomplete_buffer_,
                            incomplete_buffer_ + bytes_to_copy,
  ...
  private:
    size_t find_json_line_start(size_t target_start)
    {
        ...
        if (CompressionManager::inflate_read(
                inflate_state_.get(), search_buffer, sizeof(search_buffer) - 1, &search_bytes) == 0)
        {
            ...
            if (relative_target < search_bytes)
            {
                for (int64_t i = static_cast<int64_t>(relative_target); i >= 0; i--)
                {
                    if (i == 0 || search_buffer[i - 1] == '\n')
                    {
                        ...
                        spdlog::debug(
                            "Found JSON line start at position {} (requested {})", actual_start, target_start);
                        break;
                    }
                }
            }
    ...
    void restart_compression()
    {
        ...
        if (use_checkpoint)
        {
            if (inflate_init_from_checkpoint(inflate_state_.get(), file_handle_, checkpoint_.get()) != 0)
            {
                throw ReaderError(ReaderError::COMPRESSION_ERROR, "Failed to reinitialize from checkpoint");
            }
        }
        else
        {
...
class RawStreamingSession : public BaseStreamingSession
{
  public:
    ...
    size_t stream_chunk(char *buffer, size_t buffer_size) override
    {
        if (!decompression_initialized_)
        {
            throw ReaderError(ReaderError::INITIALIZATION_ERROR, "Raw streaming session not properly initialized");
        }
        ...
        int result = CompressionManager::inflate_read(
            inflate_state_.get(), reinterpret_cast<unsigned char *>(buffer), read_size, &bytes_read);
        ...
        spdlog::debug("Raw streamed {} bytes (position: {} / {})", bytes_read, current_position_, target_end_bytes_);
...
class StreamingSessionFactory
{
  ...
  public:
    explicit StreamingSessionFactory(dft::indexer::Indexer &indexer) : indexer_(indexer) {}

    std::unique_ptr<JsonStreamingSession>
    create_json_session(const std::string &gz_path, size_t start_bytes, size_t end_bytes)
    {
    ...
    std::unique_ptr<RawStreamingSession>
    create_raw_session(const std::string &gz_path, size_t start_bytes, size_t end_bytes)
    {
    ...
    {
        return !current || !current->matches(gz_path, start_bytes, end_bytes) || current->is_finished();
    }
...
class Reader::Impl
{
  public:
    ...
    Impl(Impl &&other) noexcept
        : gz_path_(std::move(other.gz_path_)), idx_path_(std::move(other.idx_path_)), is_open_(other.is_open_),
          indexer_(std::move(other.indexer_)), session_factory_(std::move(other.session_factory_)),
          json_session_(std::move(other.json_session_)), raw_session_(std::move(other.raw_session_))
    {
...
size_t Reader::read(const std::string &gz_path, size_t start_bytes, size_t end_bytes, char *buffer, size_t buffer_size)
{
...
size_t
Reader::read_raw(const std::string &gz_path, size_t start_bytes, size_t end_bytes, char *buffer, size_t buffer_size)
{
