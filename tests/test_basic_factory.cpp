...
TEST_CASE("Factory Pattern - Basic TAR.GZ functionality") {
    ...
    SUBCASE("IndexerFactory creates valid TAR.GZ indexer") {
        auto indexer = IndexerFactory::create(tar_gz_file, idx_file, 1024 * 1024);
        REQUIRE(indexer != nullptr);
...
TEST_CASE("Basic Reading Operations") {
    ...
    SUBCASE("Basic metadata access") {
        ...
        MESSAGE("Lines: " << reader->get_num_lines() << ", Bytes: " << reader->get_max_bytes());
    }
    
    SUBCASE("Line-based reading") {
        std::size_t num_lines = reader->get_num_lines();
        if (num_lines > 0) {
            std::string content = reader->read_lines(1, std::min(num_lines, static_cast<std::size_t>(5)));
            CHECK(!content.empty());
    ...
    SUBCASE("Byte-based reading") {
        ...
        if (max_bytes > 0) {
            ...
            std::size_t bytes_read = reader->read(0, std::min(max_bytes, static_cast<std::size_t>(1024)), 
