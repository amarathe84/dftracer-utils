...
TEST_CASE("C++ API - Error handling") {
  ...
  SUBCASE("Invalid reader creation") {
    CHECK_THROWS_AS(dftracer::utils::reader::Reader("/nonexistent/path.gz",
...
TEST_CASE("C++ API - Edge cases") {
  ...
  SUBCASE("Invalid byte range (start >= end) should throw") {
    ...
    CHECK_THROWS(
        reader.read(50, 50, buffer, sizeof(buffer)));  // Equal start and end
  }
...
TEST_CASE("C++ API - Memory safety stress test") {
  ...
  for (int i = 0; i < 3; ++i) {  // Reduced from 100 to 3 for easier debugging
    ...
    while ((bytes_read =
                reader.read(0, 4 * 1024 * 1024, buffer, sizeof(buffer))) > 0) {
...
TEST_CASE("C++ API - Exception handling comprehensive tests") {
  ...
  SUBCASE("Indexer with invalid paths should throw during build") {
    dftracer::utils::indexer::Indexer indexer("/definitely/nonexistent/path.gz",
                                              "/also/nonexistent/path.idx",
                                              mb_to_b(1.0));
    CHECK(indexer.is_valid());  // Constructor succeeds

    CHECK_THROWS_AS(indexer.build(), std::runtime_error);
    CHECK_THROWS_AS(indexer.build(),
                    std::runtime_error);  // Should throw consistently
  }
...
TEST_CASE("C++ API - Advanced reader functionality") {
  ...
  SUBCASE("Various read patterns") {
    ...
    size_t bytes_read;
    while ((bytes_read =
                reader.read_line_bytes(0, 10, buffer, sizeof(buffer))) > 0) {
      result.append(buffer, bytes_read);
    }
...
TEST_CASE("C++ API - JSON boundary detection") {
  ...
  SUBCASE("Small range should provide minimum requested bytes") {
    ...
    while ((bytes_read =
                reader.read_line_bytes(0, 100, buffer, sizeof(buffer))) > 0) {
      content.append(buffer, bytes_read);
    }
  ...
  SUBCASE("Output should not cut off in middle of JSON") {
    ...
    while ((bytes_read =
                reader.read_line_bytes(0, 500, buffer, sizeof(buffer))) > 0) {
      content.append(buffer, bytes_read);
    }
...
TEST_CASE("C++ Reader - Raw reading functionality") {
  ...
  SUBCASE("Compare raw vs regular read") {
    ...
    size_t bytes_read2;
    while ((bytes_read2 =
                reader2.read_line_bytes(0, 100, buffer2, buffer_size)) > 0) {
  ...
  SUBCASE("Raw read edge cases") {
    ...
    if (max_bytes > 10) {
      result.clear();
      while ((bytes_read = reader.read(max_bytes - 10, max_bytes - 1, buffer,
                                       sizeof(buffer))) > 0) {
        result.append(buffer, bytes_read);
      }
      CHECK(result.size() == 9);
  ...
  SUBCASE("Raw read with small buffer") {
    ...
    size_t bytes_read;
    while ((bytes_read = reader.read(0, 200, small_buffer, small_buffer_size)) >
           0) {
  ...
  SUBCASE("Raw read multiple ranges") {
    ...
    for (const auto& range : ranges) {
      if (range.second <= max_bytes) {
        std::string segment;
        size_t bytes_read;
        while ((bytes_read = reader.read(range.first, range.second, buffer,
                                         sizeof(buffer))) > 0) {
  ...
  SUBCASE("Full file read comparison: raw vs JSON-boundary aware") {
    ...
    size_t bytes_read1;
    while ((bytes_read1 = reader1.read(0, max_bytes, buffer, sizeof(buffer))) >
           0) {
...
TEST_CASE("C++ Reader - Line reading functionality") {
  TestEnvironment env(
      10000);  // Create larger test with many lines for better indexer support
  REQUIRE(env.is_valid());
  ...
  // Build index first with smaller chunk size to force checkpoint creation
  {
    ...
    INFO("Indexer created with " << checkpoints.size() << " checkpoints and "
...
TEST_CASE("C++ Advanced Functions - Error Paths and Edge Cases") {
  ...
  SUBCASE("Reader with different range sizes to trigger various code paths") {
    ...
    for (const auto& range : ranges) {
      ...
      if (end <= max_bytes) {
        ...
        size_t bytes_read;
        while ((bytes_read = reader.read(start, end, buffer, sizeof(buffer))) >
               0) {
          result.append(buffer, bytes_read);
        }
  ...
  SUBCASE("Edge case: Reading near file boundaries") {
    ...
    if (max_bytes > 10) {
      ...
      result.clear();
      while ((bytes_read = reader.read(max_bytes - 100, max_bytes - 1, buffer,
                                       sizeof(buffer))) > 0) {
        result.append(buffer, bytes_read);
      }
      ...
      if (max_bytes > 1) {
        result.clear();
        while ((bytes_read = reader.read(max_bytes - 1, max_bytes, buffer,
                                         sizeof(buffer))) > 0) {
          result.append(buffer, bytes_read);
        }
        CHECK(result.size() <= 1);
  ...
  SUBCASE("Large file handling") {
    ...
    if (max_bytes > 1000) {
      ...
      // Second range
      result.clear();
      while ((bytes_read = reader.read(500, 1500, buffer, sizeof(buffer))) >
             0) {
        result.append(buffer, bytes_read);
      }
      CHECK(result.size() <= 1000);
