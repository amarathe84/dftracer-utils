...
void test_format_indexer_creation_and_destruction(void) {
    ...
    dft_indexer_handle_t indexer = dft_indexer_create(g_test_file, g_idx_file, mb_to_b(1.0), 0);
...
void test_format_index_building(void) {
    ...
    printf("  %s format has %lu lines\n", g_format_name, (unsigned long)line_count);
...
void test_format_reader_creation_and_basic_reading(void) {
    ...
    // Build index first
    dft_indexer_handle_t indexer = dft_indexer_create(g_test_file, g_idx_file, mb_to_b(0.5), 0);
    TEST_ASSERT_NOT_NULL(indexer);
    ...
    int bytes_read;
    while ((bytes_read = dft_reader_read(reader, 0, 100, buffer, buffer_size)) > 0) {
...
void test_format_json_boundary_detection(void) {
    ...
    int bytes_read;
    while ((bytes_read = dft_reader_read_line_bytes(reader, 0, 150, buffer, sizeof(buffer))) > 0) {
        output = realloc(output, total_bytes + bytes_read);
        TEST_ASSERT_NOT_NULL(output);
        memcpy(output + total_bytes, buffer, bytes_read);
        total_bytes += bytes_read;
...
void test_tar_gz_multiple_files_detection(void) {
    ...
    char buffer[4096];
    int bytes_read;
    while ((bytes_read = dft_reader_read(reader, 0, max_bytes, buffer, sizeof(buffer))) > 0) {
    ...
    int has_additional = strstr(full_content, "\"file\": \"additional\"") != NULL;
